...
- (void)preParseCopines:(NSInteger)userId {
    NSString *sUrl = [NSString stringWithFormat:@"http://ec2-54-170-94-162.eu-west-1.compute.amazonaws.com/ios/getCopinesIds?id=%ld", (long)userId];
    ...
    if (totalIds) {
        for (NSString *artId in ids) {
            ...
            if (![queryResult count]) {
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    NSString *getArticleUrl = [NSString stringWithFormat:@"http://ec2-54-170-94-162.eu-west-1.compute.amazonaws.com/ios/getArticle?id=%@", artId];
...
- (void)parseCopines:(NSInteger)userId {
    NSString *sUrl = [NSString stringWithFormat:@"http://ec2-54-170-94-162.eu-west-1.compute.amazonaws.com/ios/getFeed?id=%ld", (long)userId];
